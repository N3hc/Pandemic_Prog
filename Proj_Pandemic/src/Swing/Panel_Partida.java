...
public class Panel_Partida extends JPanel implements ActionListener {
	...
	private void vacunasPorcentaje() {
		arrayVacuna = new int[4];
		int i = 0;
		partida.modificarVacuna("Corazón de Vell", 3);
		for (Vacuna vacuna : partida.getVacunas()) {
			arrayVacuna[i] = (int) vacuna.getPorcentaje();
			i++;
		}
	}
	...
	private void InitComponentes() {
		...
		ProgressBar[0].setValue(arrayVacuna[0]);
		...
		ProgressBar[1].setValue(arrayVacuna[1]);
		...
		ProgressBar[2].setValue(arrayVacuna[2]);
		...
		ProgressBar[3].setValue(arrayVacuna[3]);
